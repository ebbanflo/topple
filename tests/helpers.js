...
export async function joinViaLink(context, code, name) {
  const page = await context.newPage();
  await page.addInitScript((n) => localStorage.setItem('topple-name', n), name);
  await page.goto(`/index.html?t=local&debug=1&join=${code}`);
  await page.waitForSelector('#scr-lobby:not(.hidden)', { timeout: 15000 });
  return page;
}

export const state = (page) => page.evaluate(() => window.__topple.state());
export const engineState = (page) => page.evaluate(() => window.__topple.engineState());
export const setSettings = (page, s) => page.evaluate((x) => window.__topple.setSettings(x), s);
export const setScore = (page, pid, n) => page.evaluate(([p, v]) => window.__topple.setScore(p, v), [pid, n]);
export const selfId = (page) => page.evaluate(() => window.__topple.selfId);
export const buy = (page, item, target) =>
  page.evaluate(([i, t]) => window.__topple.buy(i, t), [item, target]);

export async function startGame(hostPage, pages) {
  await hostPage.click('#btn-start');
  for (const p of pages) {
    await p.waitForFunction(() => window.__topple.state().started);
  }
}

const POLL = { polling: 100 }; // background pages throttle rAF; poll by clock

export const towerState = (page) => page.evaluate(() => window.__topple.state().tower);
export const usedWords = (page) =>
  page.evaluate(() => window.__topple.state().tower.rows.map((r) => r.word));

export const waitTower = (page) =>
  page.waitForFunction(() => !!window.__topple.state().tower, null, POLL);

export const waitGameOver = (page) =>
  page.waitForFunction(() => window.__topple.state().over, null, POLL);
...
export async function climb(page, word) {
  ...
  await page.evaluate((w) => window.__topple.guess(w), word);
  await page.waitForFunction((h) => window.__topple.state().tower?.height === h + 1, before, { polling: 50 });
}

// Submit a word the host will reject, and wait for the life to come off.
export async function miss(page, word) {
  const lives = await page.evaluate(() => window.__topple.state().tower.lives[window.__topple.selfId]);
  await page.evaluate((w) => window.__topple.guess(w), word);
  await page.waitForFunction((n) => {
    const st = window.__topple.state();
